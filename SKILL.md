...
| Script | What it does |
|---|---|
| `structure_probe.py` | Single-file first pass. Ranks candidate record sizes by column-entropy, extracts ASCII/UTF-16 strings, reports global entropy and byte-frequency spikes (clues for padding, erased flash, compressed/encrypted regions). |
...
| Script | What it does |
|---|---|
| `gen_parser.py` | Emits a standalone Python parser (CSV output, stdlib-only, handles scales and YYYYMMDD/HHMMSS/unix-seconds decoding, skips blank records per the fieldmap). |
