...
A toolkit for reverse-engineering undocumented fixed-record binary file formats. Produces a runnable Python parser and a README-style format specification from any sample file, with or without a reference export of the decoded data.

Built on the observation that most device-log formats (GPS trackers, sensor loggers, telemetry dumps, game saves, dash-cam metadata) share the same shape: a stream of identically-sized records, optionally preceded by a small header. Given that shape, a handful of brute-force tricks can surface most of the fields in minutes instead of days.
...
- Anything that looks like "a uniform stream of small records" — fitness trackers, dive computers, OBD/CAN loggers, weather stations, drone flight logs, lab instruments, old game saves.

**Poor fit**

- Container formats (PNG, ZIP, WAV, ELF executables) — these have nested headers and chunks this tool doesn't model.
- Variable-length records (protobuf, length-prefixed framing).
...
```
git clone https://github.com/<your-user>/binary-format-reverser.git
...
| Script | Purpose |
| --- | --- |
| `scripts/structure_probe.py` | Rank candidate record sizes by column entropy; extract ASCII/UTF-16 strings; report byte-frequency spikes (fill patterns, erased flash, compressed regions). |
...
Supported encodings: `YYYYMMDD`, `HHMMSS`, `unix_seconds`, `ddmm`, `degrees`, `meters`, `kmh`, `enum`, plus free-form labels for documentation.
...
This tool handles fixed-record binary formats. It does not handle variable-length records, container/chunk formats (PNG, WAV, ZIP, ELF), compressed or encrypted payloads, text-mixed formats, or bit-packed fields below the byte level. The entropy probe in `structure_probe` will correctly flag regions that look compressed or encrypted; decoding them is out of scope.

It also won't identify checksums or CRCs — they look like random bytes and will be reported as unknown.

## Contributing

Extensions that would broaden the tool's reach:

- Support for variable-length records (length-prefixed or delimiter-based framing).
- Bit-packed sub-byte field extraction.
- Container-format detection (LEN-TYPE-DATA chunk walking).
- More encoding transforms (sexagesimal coordinates, fixed-point with non-decimal scale).
